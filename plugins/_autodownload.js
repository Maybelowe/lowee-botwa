...
handler.all = async function (m, { isPrems }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
       ...
       //if (!res.ok) return m.reply(eror)
        ...
        await this.sendFile(m.chat, json.nowm, '', credit, m)
    }
    ...
    if (/^.*cocofun/i.test(m.text)) {
       ...
       // if (!res.ok) return m.reply(eror)
        ...
        await m.reply(wait)
        // m.reply(util.format(json))
        await this.sendFile(m.chat, json.download, '', credit, m)
    }
    ...
    if (/^.*instagram.com\/(p|reel|tv)/i.test(m.text)) {
        igdl(url).then(async res => {
            ...
            for (let { downloadUrl, type } of json) {
                this.sendFile(m.chat, downloadUrl, 'ig' + (type == 'image' ? '.jpg' : '.mp4'), credit, m, 0, { thumbnail: await (await fetch(downloadUrl)).buffer() })
            }
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(url).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), credit, m)
            }
        }).catch(_ => _)
    }
...
`.trim(), credit, 'Audio', `.yta ${vid.url}`, 'Video', `.yt ${vid.url}`)
    }
