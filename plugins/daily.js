...
const prem = 1000
const owner = 10000
let handler = async (m, { conn, usedPrefix, isPrems, isOwner }) => {
  ...
  db.data.users[m.sender].exp += isOwner ? owner * db.data.users[m.sender].level : isPrems ? prem * db.data.users[m.sender].level : free * db.data.users[m.sender].level
